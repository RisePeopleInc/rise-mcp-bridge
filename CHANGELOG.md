...
- **Setup pages now tell users to turn on SmartProxy in their browser and open the tool first.** The tool's sign-in page loads in the user's browser, not in the bridge, so a browser that does not route through the Rise proxy makes sign-in fail (spinning tab, then an error) even when the bridge's own proxy credentials are correct. Both setup pages show a panel with a link to the proxy setup guide and an "Open <tool> ↗" button per proxy-gated tool (new window), and the step tracker gains a "Turn on SmartProxy in your browser and open the tool" step before "Start a new chat". Tools are listed in `proxyGatedTools` in `setup.go` (Metabase today; add a row per proxy-gated plugin).
